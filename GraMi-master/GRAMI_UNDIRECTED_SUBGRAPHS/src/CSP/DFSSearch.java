/**
Modified by Wenqing Lin <edwlin@me.com> to print automophism information in the output
22 Aug 2016
*/
...
public class DFSSearch
{
	...
	public void searchExistances()
	{
		...
		int nodenum = qry.getListGraph().getNodeNum();
		if ( nodenum > Settings.maxNodes) {
			System.out.println("The number of nodes is more than "+Settings.maxNodes+", and it is: "+nodenum);
			return;
		}
		
		int edgenum = qry.getListGraph().getEdgeNum();
		if ( edgenum > Settings.maxEdges) {
			System.out.println("The number of edges is more than "+Settings.maxEdges+", and it is: "+edgenum);
			return;
		}
		...
		{
			HashMap<Integer, myNode> list = autos[i].getList();
		...
		for (int i = variables.length-1; i >=0 ; i--) 
		{
			...
			System.out.println("frequency = " + result[index].getList().size() + "\n");
			HashMap<Integer, myNode> mylist = result[index].getList();

			System.out.print( "Occurrences: " );
			for (Iterator<Integer> iterator = mylist.keySet().iterator(); iterator.hasNext();) 
			{
				int nodeIndex = iterator.next();
				System.out.print( "\t" + nodeIndex );
			}	
		...
		for ( int ii=0; ii<result.length; ii++ ) {
				...
				for ( Map.Entry<Integer, myNode> myentry : result[ii].getList().entrySet() ) {
					System.out.print("\t(" + myentry.getKey() + ", " + myentry.getValue() + ") ");
				}
				System.out.print( "\n" );
			}

		System.out.println("The frequency is " + this.getFrequencyOfPattern());

			FileWriter fw;
			try
			{
				fw = new FileWriter(Settings.outputFileName, true);

				fw.write("t #\n");
				fw.write( qry.getListGraph().toString() );
